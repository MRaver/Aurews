...
function convertNewsDataToArticles() {
  ...
  if (newsData.popular) {
    ...
    if (newsData.popular.businessMain) {
      articles.push(
        createArticle(
          newsData.popular.businessMain,
          newsData.popular.businessMain.category || "Business News"
        )
      );
    }
...
function renderArticles(articles) {
  ...
  if (resultsCount)
    resultsCount.innerHTML = `<strong>${articles.length}</strong> results`;
  ...
  articles.forEach((article) => {
    ...
    dateDiv.appendChild(
      document.createTextNode(" " + article.date.toLocaleDateString())
    );
